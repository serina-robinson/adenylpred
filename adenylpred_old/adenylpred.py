...
"""
Script to predict substrate given amino acid sequence
"""
...
from lib.antismash import feature, antismash_domain, path, locations, domain, subprocessing, nrps_predictor
...
def define_arguments() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fasta", required = True, type = str, help = "Fasta directory.")
    return parser
...
def write_prediction(results):
    out_dir = "predictions.txt"
    out_file = open(out_dir, 'w')
    out_file.write("Query_name\tPrediction\tProbability")
    out_file.write("\n")
...
if __name__ == "__main__":
    ...
    domain_list = []
    create_domain_fa = fasta.read_fasta(args.fasta)
